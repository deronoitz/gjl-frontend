...
export default function AdminSettingsPage() {
  ...
  const [localSettings, setLocalSettings] = useState<{
    monthly_fee: { amount: number; currency: string };
    qris_fee: { percentage: number };
  }>({ 
    monthly_fee: { amount: 150000, currency: 'IDR' },
    qris_fee: { percentage: 0.7 }
  });
  ...
  const handleQrisFeeChange = (value: string) => {
    const numericValue = parseFloat(value) || 0;
    // Ensure value is between 0 and 100
    const clampedValue = Math.min(Math.max(numericValue, 0), 100);
    setLocalSettings(prev => ({
      ...prev,
      qris_fee: { percentage: clampedValue }
    }));
    setHasChanges(true);
  };
          ...
          <div className="grid gap-4">
            <div className="space-y-3">
              <div>
                ...
                <Input
                  id="monthlyFee"
                  value={formatCurrency(localSettings.monthly_fee.amount.toString())}
              ...
              <div>
                <Label htmlFor="qrisFee" className="text-sm font-medium">Biaya QRIS (%)</Label>
                <Input
                  id="qrisFee"
                  type="number"
                  step="0.1"
                  min="0"
                  max="100"
                  value={localSettings.qris_fee.percentage}
                  onChange={(e) => handleQrisFeeChange(e.target.value)}
                  placeholder="0.7"
                  disabled={isSaving}
                  className="mt-1"
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Persentase biaya untuk pembayaran melalui QRIS (0.7% default)
                </p>
              </div>
              ...
              <div className="bg-muted/50 rounded-lg p-3">
                <div className="text-sm text-foreground space-y-1">
                  <div>
                    <strong>Iuran Bulanan:</strong> Rp {formatCurrency(settings.monthly_fee.amount.toString())}
                  </div>
                  <div>
                    <strong>Biaya QRIS:</strong> {settings.qris_fee.percentage}%
                  </div>
                </div>
