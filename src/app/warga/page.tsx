...
import { Loader2, Users, Search, Home, Phone, User, MessageCircle } from 'lucide-react';
...
export default function WargaPage() {
  ...
  const filteredAndSortedResidents = useMemo(() => {
    const filtered = residents.filter(resident => {
      ...
      const matchesPosition = filterByPosition === 'all' || 
        (filterByPosition === 'none' && !resident.positions) ||
        (resident.positions && resident.positions.position === filterByPosition);
    ...
    <ProtectedRoute>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={filterByPosition} onValueChange={setFilterByPosition}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-600">
          Menampilkan {filteredAndSortedResidents.length} dari {residents.length} warga
        </p>
        {searchTerm && (
        ...
        <CardContent className="px-0 md:px-6">
          ...
          <div className="md:hidden space-y-3 px-4 pb-4">
            {filteredAndSortedResidents.map((resident) => (
              <Card key={resident.id} className="border border-gray-200 hover:shadow-sm py-0 transition-shadow">
                <CardContent className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center gap-2">
                        <Home className="h-4 w-4 text-gray-600" />
                        <span className="font-semibold text-base">{resident.house_number}</span>
                      </div>
                      <p className="text-sm text-gray-600 font-medium">{resident.name}</p>
                    ...
                    <div className="pt-2 border-t border-gray-100">
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => window.open(`tel:${resident.phone_number}`)}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (resident.phone_number) {
                              window.open(`https://wa.me/${resident.phone_number.replace(/\D/g, '')}`, '_blank');
                            }
                          }}
                        >
            ...
            {filteredAndSortedResidents.length === 0 && searchTerm && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  Tidak ditemukan warga yang sesuai dengan pencarian &ldquo;{searchTerm}&rdquo;
                </p>
              </div>
            )}
          </div>
          ...
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">
                Data warga akan muncul di sini
              </p>
            </div>
          </CardContent>
        </Card>
